...
export default function ShoppingPage() {
  ...
  // Inline "Clear checked" confirmation state
  ...
  const handleToggle = useCallback(
    (check_key: string, checked: boolean) => {
      toggleCheck(check_key, checked);
    },
    [toggleCheck]
  );
  ...
  const handleClearChecked = useCallback(async () => {
    ...
    if (ok) {
      showToast(`${checkedCount} item${checkedCount !== 1 ? "s" : ""} cleared`);
    } else {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-bg-card border border-border rounded-card h-[220px] animate-pulse"
            />
          ))}
        </div>
      ) : totalCount === 0 ? (
        ...
        <div className="text-center py-20 text-ink-muted">
          <CheckCheck
            className="w-14 h-14 mx-auto mb-4 text-accent opacity-60"
            strokeWidth={1.5}
          />
          <p className="text-[16px] font-medium mb-1 text-ink">
