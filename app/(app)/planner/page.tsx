...
import {
  RecipePickerAdd,
  RecipePickerEdit,
} from "@/components/planner/RecipePicker";
...
/**
 * Returns a map of DayName → Date for the week that contains `today`
 * (or is offset by `weekOffset` weeks), where the week starts on `weekStartDay`.
 */
function getWeekDates(
  ...
  const startIdx = JS_DAY_INDEX[weekStartDay];
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const name = JS_INDEX_TO_DAY[d.getDay()];
    result[name] = d;
  }
  return result;
}
...
export default function PlannerPage() {
  const { assignments, profile, loading, addAssignment, updateAssignment, removeAssignment } =
    usePlanner();
  ...
  // Week start from profile (default Monday)
  const weekStartDay: DayName = profile?.week_start_day ?? "Monday";

  // Compute ordered days and date map
  ...
  const weekDates = useMemo(
    () => getWeekDates(weekOffset, weekStartDay),
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        {/* Week navigation */}
        <div className="flex items-center gap-2 bg-bg-card border border-border rounded-[12px] px-1.5 py-1">
          <button
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] hover:bg-bg-warm transition-colors"
          >
            <CalendarDays className="w-3.5 h-3.5 text-ink-muted" strokeWidth={2} />
            <span className="text-[12px] font-semibold text-ink min-w-[76px] text-center">
              {weekLabel}
            </span>
          </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {Array.from({ length: 7 }).map((_, i) => (
            <div
              key={i}
              className="bg-bg-card border border-border rounded-card h-[280px] animate-pulse"
            />
          ))}
        </div>
      ) : (
